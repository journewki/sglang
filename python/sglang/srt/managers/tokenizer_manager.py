...
def wrap_pickle_wrapper(
    ...
    if isinstance(
        ...
        if (
            isinstance(tokenized_obj, TokenizedGenerateReqInput)
            and tokenized_obj.mm_data_mooncake is not None
        ):
            tokenized_obj.mm_data_mooncake = wrap_as_pickle(
                tokenized_obj.mm_data_mooncake
            )
